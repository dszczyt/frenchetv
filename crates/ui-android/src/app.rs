...
enum AsyncMsg {
    AuthErr(String),
    /// Operator requires mobile push approval — show a waiting indicator.
    PushAuthPending,
...
/// Minimal push-wait screen for TV: animated dots while polling.
struct PushWaitScreen {
    elapsed: f32,
}
            ...
            .show(ctx, |ui| {
                ui.centered_and_justified(|ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.label(
                            RichText::new(
                                "Appuyez sur la notification Orange sur votre téléphone.",
                            )
...
impl App {
    pub fn new(cc: &eframe::CreationContext<'_>, android_app: AndroidApp) -> Self {
        ...
        // Attempt session restore from saved credentials.
        ...
        if !kind_str.is_empty() && !username.is_empty() {
            if let Some(kind) = OperatorKind::from_config_str(&kind_str) {
                if let Some(token) = session_store::load_session(&kind_str, &username) {
    ...
    fn start_restore_session(
        ...
        self.rt.spawn(async move {
            ...
            if let Err(e) = op.restore_session(&token).await {
                log::info!("Session restore failed ({}); showing setup screen", e);
                session_store::clear_session(&kind_str, &username);
                ctx.request_repaint();
                return;
            }
    ...
    fn start_resolve_stream(&mut self, channel: Channel) {
        let tx = self.tx.clone();
        let ctx = self.egui_ctx.clone();
        let op = match &self.current_operator {
            Some(op) => op.clone(),
            None => {
                log::error!("resolve_stream called with no operator");
                return;
            }
        };
        ...
        self.rt.spawn(async move {
            ...
            match result {
                Ok(stream) => {
                    log::info!("resolve_stream: ok -> {}", stream.url);
                    let _ = tx.send(AsyncMsg::StreamOk { stream });
                }
                Err(OperatorError::InvalidCredentials) => {
                    log::warn!("resolve_stream: 401/403 -> SessionExpired");
                    let _ = tx.send(AsyncMsg::SessionExpired);
                }
                Err(e) => {
                    log::error!("resolve_stream error: {}", e);
            ...
            match msg {
                ...
                AsyncMsg::ChannelsOk {
                    ...
                    if let Some(ref token) = session_token {
                        session_store::save_session(&kind_str, &username, token);
                ...
                AsyncMsg::StreamOk { stream } => {
                    // On Android, pass directly to ExoPlayer (handles DRM natively).
                    if let Some(channel) = self.pending_channel.take() {
                        crate::player::exoplayer::launch_player(&self.android_app, &stream);
