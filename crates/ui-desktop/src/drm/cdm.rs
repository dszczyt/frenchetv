...
///
/// Only builds against the real shim on Linux/macOS: `cdm_shim.cpp` hand-replicates
/// Chromium's `cdm::ContentDecryptionModule` vtable against the Itanium C++ ABI,
/// which matches how `libwidevinecdm.so`/`.dylib` are built. Google's Windows
/// `widevinecdm.dll` is MSVC-ABI, so the same vtable layout would not match —
/// porting the shim's `dlopen` calls to `LoadLibrary` would make it link, then
/// crash (or worse) on the first virtual call. See `stub` below for the
/// Windows/other behavior: DRM-gated channels get a plain stream error, same as
/// `bouygues.rs` does for its PFS-gated live streams.
...
mod real {
    ...
    #[repr(C)]
    struct RawDecryptInput {
        data: *const u8,
        data_size: u32,
        encryption_scheme: u32, // 1 = CENC (AES-128-CTR), 2 = CBCS (AES-128-CBC)
        key_id: *const u8,
        key_id_size: u32,
        iv: *const u8,
        iv_size: u32,
        subsamples: *const SubsampleEntry,
        num_subsamples: u32,
        timestamp: i64,
    }
    ...
    #[repr(C)]
    struct RawDecryptOutput {
        data: *mut u8,
        data_size: u32,
        status: c_int, // 0 = kSuccess
    }
    ...
    extern "C" {
        ...
        fn cdm_decrypt(state: *mut CdmStateOpaque, inp: *const RawDecryptInput)
            -> RawDecryptOutput;
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
pub use real::CdmHandle;

/// Stub used on platforms without a working CDM shim (currently: Windows).
/// `open()` always fails, so no instance of `CdmHandle` ever exists here —
/// the other methods are unreachable and only need to type-check.
#[cfg(not(any(target_os = "linux", target_os = "macos")))]
mod stub {
    use anyhow::{bail, Result};

    pub struct CdmHandle(std::convert::Infallible);

    impl CdmHandle {
        pub fn open(_lib_path: &str) -> Result<Self> {
            bail!("Widevine DRM playback isn't available on this platform yet")
        }
        ...
        pub fn create_session(&mut self, _pssh: &[u8]) -> Result<(String, Vec<u8>)> {
            match self.0 {}
        }

        pub fn update_session(&mut self, _session_id: &str, _response: &[u8]) -> Result<()> {
            match self.0 {}
        }
