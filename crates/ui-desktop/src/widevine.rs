...
const X86_BOARDS: &[&str] = &["link", "peppy", "falco", "wolf", "clapper", "squawks"];
...
async fn download_from_omaha(client: &reqwest::Client) -> Result<()> {
    ...
    tracing::info!(
        "widevine: Omaha installed {} bytes → {}",
        cdm.len(),
        cdm_path().display()
    );
    Ok(())
...
async fn fetch_recovery_manifest(client: &reqwest::Client) -> Result<Vec<RecoveryEntry>> {
    ...
    let json: Vec<serde_json::Value> = client
        ...
        .send()
        .await
        .context("GET recovery manifest")?
        .json()
        .await
        .context("parse recovery manifest")?;

    let entries: Vec<RecoveryEntry> = json
        .iter()
        .filter_map(|e| {
            Some(RecoveryEntry {
...
async fn try_stream_to_file(client: &reqwest::Client, url: &str, dest: &Path) -> Result<()> {
...
fn gpt_partition_range(bin_path: &Path, part_num: usize) -> Result<(u64, u64)> {
    ...
    f.seek(SeekFrom::Start(entry_off))
        .context("seek to partition entry")?;
    ...
    tracing::debug!(
        "widevine: partition {} at offset {} size {} MB",
        part_num,
        offset,
        size / 1_000_000
    );
...
async fn download_from_recovery(client: &reqwest::Client) -> Result<()> {
    ...
    let entry = select_recovery_entry(&entries).ok_or_else(|| {
        anyhow::anyhow!(
    ...
    if let Err(e) = std::fs::remove_dir_all(&tmp) {
        tracing::warn!(
            "widevine: failed to clean up temp dir {}: {}",
            tmp.display(),
            e
        );
    }
...
async fn recovery_install_inner(
    ...
    .await
    .context("spawn_blocking extract_bin")??;
    ...
    .await
    .context("spawn_blocking squashfs")??;
    ...
    tracing::info!(
        "widevine: CDM installed from Chrome OS recovery → {}",
        cdm_path().display()
    );
    Ok(())
}
